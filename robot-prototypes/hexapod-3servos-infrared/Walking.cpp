...
void initializeServos()
{
  ...
  AntsyServo[CENTER_SERVO].attach(2, CENTER_MIN, CENTER_MAX);      // attaches/activates the center servo to pin 9 sets min/max values
  AntsyServo[RIGHT_SERVO].attach(3, RIGHT_MIN, RIGHT_MAX);  // attaches/activates the right servo to pin 10, sets min/max values
  AntsyServo[LEFT_SERVO].attach(4, LEFT_MIN, LEFT_MAX);  // attaches/activates the left servo to pin 11, sets min/max values
}
...
void Stop()
{
  // Retorna todos os servos para a posição central
  centerServoValue = CENTER_CENTER;
  rightServoValue = RIGHT_CENTER;
  leftServoValue = LEFT_CENTER;
  
  // Usa a função SetServo já existente para mover os servos
  // Use um tempo de transição razoável, por exemplo 500ms
  SetServo(500);
  delay(500);
}
