...
QTRSensors qtrrc;//((unsigned char[]) {  14, 15, 16, 17, 18, 19} ,NUM_SENSORS, TIMEOUT, EMITTER_PIN); // sensor connected through analog pins A0 - A5 i.e. digital pins 14-19
...
void setup()
{
  ...
  //((unsigned char[]) {  14, 15, 16, 17, 18, 19} ,NUM_SENSORS, TIMEOUT, EMITTER_PIN); // sensor connected through analog pins A0 - A5 i.e. digital pins 14-19
  
  qtrrc.setTypeAnalog();
  qtrrc.setSensorPins((const uint8_t[]){A0, A1}, 2);
...
void loop()
{
  ...
  uint16_t position = qtrrc.readLineBlack(sensors);
